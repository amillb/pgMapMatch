...
3. Provide details on the column names of the streets table (if you are not using osm2po)

Save this file as `config.py`. `config_template.py` will be used only if `config.py` is not found.
"""

# postgres connection information
pgInfo = {'db':'your_database_name', 
          'schema':'your_schema_name',    # schema where your GPS traces and streets table exist
          'user':'your_postgres_username', 
          'host':'localhost_or_IP_address',
          'requirePassword':True  # will you be prompted for a password? Normally set to False for localhost
          }

maxNodes = 250          # how many nodes pgrouting can route at once. If you have lots of memory, increase this. If you get a memory error, reduce it

# All these parameters assume meters, but they must match the units of your projection
...
fwayQuery = 'clazz<15 OR kmh>=100'      # SQL-compliant query that identifies freeways (with the higher gps error tolerance)
fwayCols = 'clazz'                      # comma-separated list of columns that are needed in fwayQuery, but are not listed above
