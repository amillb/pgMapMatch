...
import itertools, time, datetime, random, math, os
...
class traceCleaner():
    def  __init__(self, traceTable, idName, geomNameOld, geomNameNew=None, maxSpeed=maxSpeed,logFn=None):
        ...
        geomNameNew: name of the geometry column to populate. Anything in this column will be overwritten. If None, is geomNameOld
        logFn: optional filename to create a log file        
        """

        self.db = mmt.dbConnection(pgLogin=pgInfo)
        if self.db.execfetch('SELECT COUNT(%s)=COUNT(DISTINCT %s) FROM %s' % (idName, idName,traceTable))[0][0] is False:
        ...
        self.srid = self.db.execfetch('SELECT ST_Srid(%s) FROM %s LIMIT 1' % (self.geomNameOld, self.traceTable))[0][0]
    ...
    def getTracesDf(self, traceId=None, numLags=5):
        """Gets a pandas dataframe of traces. Optionally limits to the ids specified in the traceId list"""
    ...
    def fetchPtsToDrop(self, traceId=None, numLags=5):
        ...
        Max speed must be in the units of your projection (normally meters), per second"""
        ...
        # Finally, select any 'singletons' where there is one errant point that's not picked up by the lagged values
...
class mapMatcher():
    def  __init__(self, streetsTable, traceTable=None, idName=None, geomName=None, newGeomName='matched_line', cleanedGeomName=None, qualityModelFn='mapMatching_coefficients.txt', db=None):
        ...
        self.cmdDict = {'streetsTable':streetsTable, 'streetIdCol':streetIdCol, 'streetGeomCol':streetGeomCol,
              ...
              'km':streetLengthCol,'kmh':speedLimitCol, 'fwayCols':'' if fwayCols.strip()=='' else ','+fwayCols,
    ...
    def matchGPXTrace(self,fn):
        ...
        If there are multiple segments, only matches the first one (for now...this is easy to modify)"""
        ...
        self.startEndPts = ['ST_Transform(ST_SetSRID(ST_MakePoint('+str(pt.longitude)+','
                             +str(pt.latitude)+'),4326),'+self.streets_srid+')' for pt in [traceSegment.points[0],traceSegment.points[-1]]]
    ...
    def matchTrace(self):
        ...
        # coo matrix from lists is fastest way to build a sparse matrix, rather than assigning to lil or dok matrix directly
    ...
    def getPtsDf(self):
        ...
        self.ptsDf['rownum'] = range(len(self.ptsDf))
    ...
    def writeMatchToPostgres(self,edgeIdCol='edge_ids',writeEdgeIds=True,writeGeom=True,writeMatchScore=True,writeLLs=False):
        ...
        cDict = dict(self.cmdDict,**{'edgeIdCol':edgeIdCol,
                        'edges':str(self.bestRoute), 'traceId':self.traceId, 'll_dist_mean':self.LL[0],'ll_dist_min':self.LL[1],
                        'll_topol_mean':self.LL[2], 'll_topol_min':self.LL[3],'ll_distratio_mean':self.LL[4], 'll_distratio_min':self.LL[5]})
    ...
    def updateCostMatrix(self):
       ...
       # TO DO: restricted pgrouting query may improve performance
       # E.g.   WHERE geom_way  && (select st_buffer(lbuff_geom,5000) as myarea FROM trace_table where trip_id = XXX)
       # See http://docs.pgrouting.org/latest/en/pgRouting-concepts.html#performance
        ...
        if 1:
            ...
            nodesToDoList = [allNodesToDo[x:x+maxNodes] for x in xrange(0, len(allNodesToDo), maxNodes)]
    ...
    def fillRouteGaps(self,route,uturns):
        ...
        for ii, (edge, node, uturn,ptId) in enumerate(itertools.izip(edgeList[1:],nodeList[1:],uturnList,keepPts[1:-1])):
    ...
    def addQualityColumns(self,columns=None,forceUpdate=False):
        ...
        colsToAdd = [('pingtime_max','real'),('pingtime_mean','real'),('gpsdist','real'),('matchdist','real'),('frechet_dist','real')]        
    ...
    def transProbSameEdge(self,rr1,rr2,dir,sl):
        """Returns input to likelihood functions for movement along an edge (rr1.edge==rr1.edge)"""
    ...
    def transProb(self,rr1,rr2,dir1,dir2,sl):
        """Returns input to likelihood functions for transition between edges (rr1.edge!=rr1.edge)"""
        if dir1==0:
        ...
        return max(cratio,1e-10), max(dratio,1), uturn  # the values will be in a sparse matrix, so zeros aren't distinguishable from empty arrays
    ...
    def viterbi(self):
        ...
        #if obs is None: obs=xrange(self.obsProb.shape[1])
...
class qualityPredictor():
    ...
    def estimateQualityModel(self,holdback=0.3,threshold_score=0.7): #,pingtime_threshold=30):
        """
        ...
        drops traces with pingtime_mean>pingtime_threshold
        """
        ...
        models['model1'] = ['pingtime_mean','pingtime_max','frechet_dist','gpsMatchRatio', 'matchGpsRatio','ll_dist_mean','ll_dist_min','ll_topol_mean','ll_topol_min','ll_distratio_mean','ll_distratio_min']  # all variables
    ...
    def scatterplots(self,df):
        ...
        yvars = ['log_pingtime_max','frechet_dist','gpsMatchRatio', 'matchGpsRatio','ll_dist_mean','ll_dist_min','ll_topol_mean','ll_topol_min','ll_distratio_mean','ll_distratio_min']
    ...
    def boxplots(self,df):
        ...
        yvars = ['log_pingtime_mean','log_pingtime_max','frechet_dist','gpsMatchRatio', 'matchGpsRatio','gpsMatchRatio_highfreq','ll_dist_mean','ll_dist_min','ll_topol_mean','ll_topol_min','ll_distratio_mean','ll_distratio_min']
