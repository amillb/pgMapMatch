...
"""
import platform, os, math, psycopg2, psycopg2.extras
import numpy as np
...
def getPgLogin(user=None, db=None, host=None, requirePassword=False, forceUpdate=False):
    ...
    user = raw_input('Username for database:') if user is None else user
    db = raw_input('Database name:') if db is None else db
...
class dbConnection():
    def __init__(self, user=None, db=None, host=None, requirePassword=True, pgLogin=None, schema=None, role=None,curType='default', verbose=True, logger=None):
        ...
        # Connect to database
        coninfo = ' '.join([{'db':'dbname','pw':'password'}.get(key,key)+' = '+val   for key,val in self.pgLogin.items()])
    ...
    def execfetchDf(self, cmd):
        ...
        except: # probably an older version of pandas
    ...
    def addColumns(self,columns,table,skipIfExists=False,dropOld=False):
        ...
        if isinstance(columns, tuple): columns=[columns]
        if skipIfExists: 
    ...
    def merge_table_into_table(self,from_table, targettablename, commoncolumns, drop_old=True, drop_incoming_duplicates=False, recreate_indices=True, forceUpdate=True):
        """
        ...
        if drop_old: self.execute(' DROP TABLE old_'+targettablename+';')
        self.execute(' ALTER TABLE new_'+targettablename+' RENAME TO  '+targettablename+';')

        if recreate_indices:
            for idxCmd in indices: self.execute(idxCmd[0])

    def update_table_from_array(self,data, targettablename, joinOnIndices=False, temptablename=None,  if_exists='replace',  preserve_uppercase=False, drop_incoming_duplicates=False):
        """
    ...
    def create_indices(self,table,key=None, non_unique_columns=None, geom=False, skip_vacuum=False, forceUpdate=True):
